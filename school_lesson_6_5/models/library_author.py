...
class LibraryAuthor(models.Model):
    ...
    @api.model
    def calculate_author_anniversary(self, notify_type: str = 'todo') -> None:
        ...
        :param notify_type: can have two value toodo or meeting, determine which activity type to create.
        :return None:
        """
        activity_types = {
        ...
        if notify_type not in activity_types.keys():
            return None
        authors = self.env['library.author'].search([('birth_date', '!=', False)])
        for author in authors:
            # Get only those authors whose century anniversary
            # is in the current month
            if (
                    (fields.Date.today().year - author.birth_date.year) % 100
                    and fields.Date.today().month == author.birth_date.month
            ):
                self.env['mail.activity'].create({
                    'res_model_id': self.env.ref('school_lesson_6_2.model_library_author').id,
                    'res_id': author.id,
                    'activity_type_id': self.env.ref(activity_types[notify_type]).id,
                    'user_id': self.env.ref('base.user_admin').id,
                    'summary': _("%s anniversary is coming soon.", author.display_name),
